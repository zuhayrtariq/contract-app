...
export class ContractsService {
  ...
  async getOne(contractNo: number, withCoff: boolean = false, coffArchived: boolean = false) {
   
    const queryBuilder = this.contractRepo.createQueryBuilder('contract');
    queryBuilder.where('contract.contractNo = :contractNo', { contractNo })
    if (!withCoff)
      return queryBuilder.getOne()
    coffArchived ? (queryBuilder.leftJoinAndSelect('contract.callOffs', 'callOff')) : (
      queryBuilder.leftJoinAndSelect('contract.callOffs', 'callOff', 'contract.contractNo = callOff.contractNo AND callOff.archived = 0')
    )
    return queryBuilder.getOne()
  }
  getAll(dto: GetContractDto) {
    const { sectionCode, archived, emailAlerts, contractCurrency, endDateGT, endDateLT, startDateGT, startDateLT, vendorCode, withCoff, coffArchived } = dto;
    const queryBuilder = this.contractRepo.createQueryBuilder('contract');
    sectionCode && queryBuilder.andWhere('contract.sectionCode = :sectionCode', { sectionCode });

    ('archived' in dto) && queryBuilder.andWhere('contract.archived = :archived', { archived });

    ('emailAlerts' in dto) && queryBuilder.andWhere('contract.emailAlerts = :emailAlerts', { emailAlerts });
    ...
    if (withCoff) {
      coffArchived ? (queryBuilder.leftJoinAndSelect('contract.callOffs', 'callOff')) : (queryBuilder.leftJoinAndSelect('contract.callOffs', 'callOff', 'contract.contractNo = callOff.contractNo AND callOff.archived = 0'))
    }
    return queryBuilder
      .getMany();
  }
  ...
  async getContractByValidity(sectionCode?: string) {
    ...
    contractsExpiring.andWhere('contract.archived = 0');
    contractsActive.andWhere('contract.archived = 0');
    contractsExpired.andWhere('contract.archived = 0');
    if (sectionCode) {
  ...
  async getContractsValue(sectionCode?: string) {
    ...
    const contractData: any = await this.getAll({archived : 0});
    const usdPkr = 290;
    const usdEur = 1.1;
  ...
  async getContractByStatus(sectionCode?: string) {
    ...
    activeContracts.andWhere('contract.archived = 0');
    criticalContracts.andWhere('contract.archived = 0');
    if (sectionCode) {
