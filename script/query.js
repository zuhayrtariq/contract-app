...
const getContractsExpiring = async(sectionCode) => {
  ...
  const expiryDate = getDateAfter(process.env.Contract_Expiry_Limit_Months,'months');
...
const getCoffWithFileName = async(sectionCode) =>{
  const query = `SELECT coffNo,coffFileName,sectionCode From call_off LEFT JOIN contract on contract.contractNo = call_off.contractNo where sectionCode = '${sectionCode}'`;
  return await getQueryResult(query);
}

const getSectionCodes = async() =>{
  const query = 'SELECT sectionCode FROM section';
  return await getQueryResult(query)
}

const updateFileName = async(coffNo,coffFileName) =>{
  const query = `Update call_off Set coffFileName = '${coffFileName}' WHERE coffNo = ${coffNo}`
  return await queryMutation(query)
}

const updateFileNameToNull = async(coffNo) =>{
  const query = `Update call_off Set coffFileName = null WHERE coffNo = ${coffNo}`
  return await queryMutation(query)
}
